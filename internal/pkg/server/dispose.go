				...
				if machineNum == 1 {
					...
					for eventId, requestTimeList := range requestTimeListMap {
						...
						if sceneTestResultDataMsg.Results[eventId].TotalRequestTime != 0 {
							...
							qps := fmt.Sprintf("%.2f", float64(len(requestTimeList))-float64(index))
							sceneTestResultDataMsg.Results[eventId].Qps, _ = strconv.ParseFloat(qps, 64)
						}
					}
...
//	consumer, consumerErr := sarama.NewConsumer([]string{host}, sarama.NewConfig())
//	if consumerErr != nil {
//		log2.Logger.Error("topic  :"+"stressReport"+", 创建消费者失败:", consumerErr)
//	}
//	topic := "stressReport"
//	partitions, consumerErr := consumer.Partitions(topic)
//	if consumerErr != nil {
...
//			partition, partitionConsumerErr := consumer.ConsumePartition("stressReport", p, sarama.OffsetNewest)
//			if partitionConsumerErr != nil {
//				log2.Logger.Error("创建消费者失败：", partitionConsumerErr)
//				consumerMap.Delete(p)
//				continue
//			}
//			go ReceiveMessageCopy(partition, consumerMap, p, saramaConfig, host, topic)
//		}
//	}
//}
//func ReceiveMessageCopy(partition sarama.PartitionConsumer, consumerMap *sync.Map, p int32, config *sarama.Config, host, topic string) {
//	defer partition.AsyncClose()
...
//						sceneTestResultDataMsg.Results[eventId].Qps = qps / 3
//					}
//					sceneTestResultDataMsg.TimeStamp = time.Now().Unix()
//					err = es.InsertTestData(sceneTestResultDataMsg)
//					if err != nil {
...
//						return
//					}
//					ca, errNewClusterAdmin := sarama.NewClusterAdmin([]string{host}, config)
...
//						log2.Logger.Error("删除top："+topic+"错误：", errNewClusterAdmin)
//					}
//					return
//				}
//				if sceneTestResultDataMsg.Results == nil {
